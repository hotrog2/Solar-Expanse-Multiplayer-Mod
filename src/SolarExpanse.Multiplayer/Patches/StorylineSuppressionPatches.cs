...
internal static class MissionsWindowStorylinePatches
{
    [HarmonyPrefix]
    [HarmonyPatch("Show")]
    private static bool ShowPrefix(global::Game.UI.Windows.Windows.MissionsWindow __instance)
    {
        if (!StorylineSuppressionState.Enabled)
        {
            return true;
    ...
    private static bool RefreshUiPrefix(global::Game.UI.Windows.Windows.MissionsWindow __instance)
    {
        if (!StorylineSuppressionState.Enabled)
        {
            return true;
...
[HarmonyPatch(typeof(global::Game.UI.Windows.Windows.PlanMissionWindow))]
internal static class PlanMissionWindowStorylinePatches
{
    [HarmonyPrefix]
    [HarmonyPatch("SetData", new[] { typeof(object) })]
    private static bool SetDataObjectPrefix(global::Game.UI.Windows.Windows.PlanMissionWindow __instance)
    {
        if (!StorylineSuppressionState.Enabled)
        {
            return true;
    ...
    [HarmonyPrefix]
    [HarmonyPatch("SetData", new[] { typeof(global::Game.Info.ObjectInfo), typeof(global::Game.UI.Windows.Windows.PlanMissionWindow.EStageWindow) })]
    private static bool SetDataObjectStagePrefix(global::Game.UI.Windows.Windows.PlanMissionWindow __instance)
    {
        if (!StorylineSuppressionState.Enabled)
        {
            return true;
    ...
    [HarmonyPrefix]
    [HarmonyPatch("SetPMParameterForCode")]
    private static bool SetPmParameterForCodePrefix() => !StorylineSuppressionState.Enabled;
}

[HarmonyPatch(typeof(global::Game.UI.Windows.Windows.ObjectInfoWindow))]
internal static class ObjectInfoWindowStorylinePatches
{
    [HarmonyPostfix]
    [HarmonyPatch("GetRowMissionList")]
    private static void GetRowMissionListPostfix(ref System.Collections.Generic.List<global::Game.UI.Windows.Elements.ObjectInfoElements.RowMissionData> __result)
    {
        if (!StorylineSuppressionState.Enabled)
        {
            return;
        ...
        __result = new System.Collections.Generic.List<global::Game.UI.Windows.Elements.ObjectInfoElements.RowMissionData>();
    }
}

[HarmonyPatch(typeof(MissionInfoManager))]
internal static class MissionInfoManagerStorylinePatches
{
    [HarmonyPrefix]
    [HarmonyPatch("CreateMissionInfo")]
    private static bool CreateMissionInfoPrefix(ref global::Game.Info.MissionInfo __result)
    {
        if (!StorylineSuppressionState.Enabled)
        {
            return true;
    ...
    [HarmonyPrefix]
    [HarmonyPatch("AddMissionInfo")]
    private static bool AddMissionInfoPrefix() => !StorylineSuppressionState.Enabled;

    [HarmonyPrefix]
    [HarmonyPatch("AddPMMissionParameterDataSave")]
    private static bool AddPmMissionParameterDataSavePrefix() => !StorylineSuppressionState.Enabled;

    [HarmonyPostfix]
    [HarmonyPatch("GeListPMMissionParameterDataSave")]
    private static void GeListPmMissionParameterDataSavePostfix(ref System.Collections.Generic.List<global::Game.PMMissionParameterDataSave> __result)
    {
        if (!StorylineSuppressionState.Enabled)
        {
            return;
        ...
        __result = new System.Collections.Generic.List<global::Game.PMMissionParameterDataSave>();
    }
}
